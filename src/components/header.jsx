    ...
    return (
        <header className="text-center px-2 py-3 bg-white dark:bg-black grid grid-cols-[1fr_auto_1fr] items-center  gap-1 border-b-1">
            {user.data?.username && <BurgerMenu>
                <Link className="button-like w-full" to="/settings"><IoSettingsOutline className="mr-7 absolute left-6" size={20} />Indstillinger</Link>
                {user.data?.role === 'admin' && <Link className="button-like w-full" to="/admin"><MdOutlineAdminPanelSettings  className="mr-7 absolute left-6" size={20} />Admin</Link>}
                <button onClick={user.logout} className="cancel w-full mr-1"><SlLogout className="mr-7 absolute left-6" size={20} />Log ud</button>
            </BurgerMenu> }
